...
async def get_content(request: Request, slug: str, db: AsyncSession = Depends(get_session)):
    ...
    return render_template("content.html", request, id=content.id, slug=content.slug,
                           created_at=content.created_at.strftime("%Y-%m-%d"), description=content.description,
                           image_id=content.image_id or "", title=content.title, body=html)
